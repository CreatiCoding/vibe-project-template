...
standalone 대신, PnP 의존성 구조를 그대로 활용하여 단일 스테이지에서 빌드와 실행을 처리합니다:
...
핵심: builder/runner를 분리하지 않고 `deps`를 확장한 단일 스테이지에서 빌드 후 바로 실행합니다. `.pnp.cjs`, `.yarn/cache` 등 PnP 런타임이 유지된 상태이므로 `yarn start`가 정상 동작합니다.
