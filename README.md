...
standalone 대신, PnP 의존성 구조를 그대로 활용합니다:

```dockerfile
# deps 스테이지에서 yarn install
FROM deps AS builder
COPY tsconfig.json ./
COPY services/web services/web
RUN yarn workspace @my-app/service build

# deps를 그대로 확장하여 PnP 런타임 유지
FROM deps AS runner
COPY --from=builder /app/services/web/.next services/web/.next
CMD ["yarn", "workspace", "@my-app/service", "start"]
```

핵심: runner 스테이지가 `deps`를 확장(`FROM deps`)하여 `.pnp.cjs`, `.yarn/cache` 등 PnP 런타임이 유지된 상태에서 `yarn start`를 실행합니다.
